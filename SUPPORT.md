...
Griffel is an experimental preview. There is no service-level agreement, paid support channel, or guarantee that issues will be fixed.

## Before Asking For Help

- Make sure you can build the app with `./build.sh --debug`.
- Check that your OpenAI API key is entered in the app settings — or, if you run Sicherer Lokaler Modus, that both on-device models are downloaded.
