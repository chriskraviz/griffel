...
struct LocalTranscriptionModel: Identifiable, Hashable {
    ...
    /// WhisperKit model folders carry their download size in the name
    /// (`openai_whisper-small_216MB`). Surfaced so the install button can say
    /// what it is about to download. Nil when the name has no size suffix.
    static func downloadSizeLabel(for modelName: String) -> String? {
        guard modelName.hasSuffix("MB"),
              let underscore = modelName.lastIndex(of: "_") else { return nil }
        let digits = modelName[modelName.index(after: underscore)..<modelName.index(modelName.endIndex, offsetBy: -2)]
        guard !digits.isEmpty, digits.allSatisfy(\.isNumber) else { return nil }
        return "\(digits)\u{202F}MB"
    }
...
actor LocalTranscriptionService {
    ...
    func transcribe(
        ...
        let text = joinedText(from: results)
    ...
    /// Transcribes raw 16 kHz mono samples. Used for live partial transcripts
    /// while recording; an empty result is returned as-is instead of throwing
    /// because partials are cosmetic.
    func transcribeSamples(
        ...
        return joinedText(from: results)
    }
    ...
    private func decodeOptions(
        ...
        // Bias decoding towards glossary terms via prompt tokens; the decoder
        // trims overlong prompts and drops special tokens itself.
        if !vocabularyHints.isEmpty, let tokenizer = pipeline.tokenizer {
            let hintText = " Eigennamen und Begriffe: " + vocabularyHints.joined(separator: ", ") + "."
            let tokens = tokenizer.encode(text: hintText)
            if !tokens.isEmpty {
    ...
    private func joinedText(from results: [TranscriptionResult]) -> String {
        results
            .map(\.text)
            .joined(separator: " ")
